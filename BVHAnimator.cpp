...
void BVHAnimator::solveLeftArm(int frame_no, float scale, float x, float y, float z)
{
    ...
    // -------------------------------------------------------
	glm::vec3 goal(x, y, z);
	float touchError = 0.02;

	glm::vec3 endPosition(lhand->matrix[3]);

	float max = glm::distance(rhand->matrix[3], rarm->matrix[3]);
	float maxDistance = max + touchError;
	float distanceFromShoulder = glm::distance(glm::vec3(larm->matrix[3]), endPosition);
	bool reachable = true;
	...
	//Maximum time to compute to reduce computation overhead - 2s
	int maxTime = 2 * 1000000;
	
	//If the goal is further than touch error, reachable and it's within computation time
	while (glm::distance(endPosition, goal)>=touchError && reachable && clock()-start_time<maxTime) {
			_bvh->matrixMoveTo(frame_no, scale);

			glm::vec3 armPosition(larm->matrix[3]);
			glm::vec3 lforearmPosition(lforearm->matrix[3]);
			endPosition=glm::vec3(lhand->matrix[3]);

			glm::vec3 delta = (goal - endPosition);
			//Compute Jacobian columns analitycally
			glm::vec3 dsdt1 = glm::cross(glm::vec3(1, 0, 0), endPosition - armPosition);
			glm::vec3 dsdt2 = glm::cross(glm::vec3(0, 1, 0), endPosition - armPosition);
			glm::vec3 dsdt3 = glm::cross(glm::vec3(0, 0, 1), endPosition - armPosition);
			glm::vec3 dsdt4 = glm::cross(glm::vec3(0, 1, 0), endPosition - lforearmPosition);

			glm::mat4x3 Jacobian = glm::mat4x3(dsdt1, dsdt2, dsdt3, dsdt4);

			glm::mat3x4 transpose = glm::transpose(Jacobian);
			glm::mat3x4 pseudoinverse = transpose * glm::inverse((Jacobian * transpose));
	
			glm::vec4 deltat = pseudoinverse * delta;
